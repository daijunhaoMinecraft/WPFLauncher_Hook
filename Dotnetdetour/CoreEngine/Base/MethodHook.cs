    ...
    public class MethodHook
    {
        ...
        private static void InstallInternal(bool isInstall, Assembly[] assemblies)
        {
            foreach (var detour in destAndOris)
            {
                ...
                var hookMethodAttribute = CustomAttributeExtensions.GetCustomAttribute<HookMethodAttribute>((MemberInfo)hookMethod);
                ...
                {
                    // 处理泛型方法
                    rawMethod = ((MethodInfo)rawMethod).MakeGenericMethod(Enumerable.Select<ParameterInfo, Type>(hookMethod.GetParameters(), o =>
                    {
                        var rt = o.ParameterType;
                        var attr = CustomAttributeExtensions.GetCustomAttribute<RememberTypeAttribute>((ParameterInfo)o);
