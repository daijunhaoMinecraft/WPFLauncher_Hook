...
public static class LocalProxyListener
{
        ...
        new Thread(() =>
        {
            try
            {
                ...
                while (_active)
                {
                    ...
                    // 1. 分配 ID
                    var connId = WebRtcVar.ConnIdManager.Allocate();

                    // 2. 获取正确的 PeerId (优先使用 TargetPeerId)
                    var pid = WebRtcVar.TargetPeerId;

                    if (WpfConfig.EnableVerboseLogging) Console.WriteLine($"[LocalProxy] 新连接! ConnId={connId}, Target={pid}");
    ...
    {
        var ptr = WebRtcVar.getIntPtrFromPeerId(pid);
        if (ptr != null)
            try
            {
                ...
                if (WpfConfig.EnableVerboseLogging) Console.WriteLine($"[LocalProxy] 成功向 {pid} 发送 Mux 握手信号");
            }
            catch (Exception ex)
            {
