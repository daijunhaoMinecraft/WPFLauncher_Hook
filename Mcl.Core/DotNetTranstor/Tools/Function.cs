...
internal class Function
{
    ...
    public static string GetName(string values)
    {
        ...
        try
        {
            text = JsonConvert.DeserializeObject<JObject>(JsonConvert.DeserializeObject<JObject>(values)["entity"]
    ...
    public static void ClientLog(string log, ConsoleColor color = ConsoleColor.Gray)
    {
        ...
        var text = string.Format("[INFO][{0}]{1}", DateTime.Now, log);
    ...
    {
        Console.ForegroundColor = color;
        var text = "[ERROR][" + DateTime.Now + "]" + log;
        Console.ForegroundColor = ConsoleColor.Gray;
        DebugPrint.LogDebug_NoColorSelect(text);
    }
    ...
    public static string[] GetAllFiless(string path)
    {
        ...
        var strings = Directory.GetFiles(path, "*", SearchOption.AllDirectories).ToList();
        for (var i = 0; i < strings.Count; i++)
            if (Directory.Exists(strings[i]))
                strArray.Concat(GetAllFiless(strings[i]));
            else
