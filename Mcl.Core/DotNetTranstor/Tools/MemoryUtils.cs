...
public class MemoryUtils
{
    ...
    public static int GetPerfectMemory()
    {
        try
        {
            ...
            if (!GlobalMemoryStatusEx(memStatus)) return 4096; // 获取失败默认给 4GB

            var totalMemoryMB = (long)(memStatus.ullTotalPhys / 1024 / 1024);
            ...
            var osReserve = totalMemoryMB <= 8192 ? 2048 : 3072;

            // 2. 计算理论最大值 (总内存 - 系统保留)
            var maxAvailable = totalMemoryMB - osReserve;
