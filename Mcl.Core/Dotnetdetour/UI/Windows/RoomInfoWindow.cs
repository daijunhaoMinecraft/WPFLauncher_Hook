...
public class RoomInfoWindow : System.Windows.Window
{
    ...
    private void InitializeWindow()
    {
        ...
        Mcl.Core.Dotnetdetour.UI.Themes.FluentTheme.Apply(this);
    ...
    private void UpdateRoomInfo(EntityResponse<LobbyGameRoomEntity> roomInfo)
    {
        ...
        var ownerInfo = X19Http.GetPlayerInfo(entity.owner_id);
    ...
    public void AddLogEntry(string message, ConsoleColor color = ConsoleColor.Black)
    {
        ...
        while (_logList.Items.Count > 100) _logList.Items.RemoveAt(_logList.Items.Count - 1);
    }
    ...
    public void UpdateRoomInfoFromJson(JObject roomInfo)
    {
        ...
        try
        {
            ...
            {
                var gameStatus = roomInfo["game_status"].ToObject<int>();
            ...
            {
                var ownerId = roomInfo["owner_id"].ToString();
                var ownerInfo = X19Http.GetPlayerInfo(ownerId);
            ...
            {
                var hasPassword = roomInfo["password"].ToObject<int>() != 0;
            ...
            {
                var allowSave = roomInfo["allow_save"].ToObject<int>() != 0;
    ...
    public async void HandlePlayerJoin(string playerId)
    {
        try
        {
            ...
            {
                var playerName = playerInfo["entity"]["name"].ToString();
