...
namespace System.Runtime.CompilerServices
{
    ...
    public class HookBootstrapper
    {
        ...
        private static void ExitProcess()
        {
            ...
            try
            {
                PluginLog.Info("Core", "程序即将退出, 正在执行清理操作...");
                PluginLog.Info("Core", "检查是否进入联机大厅房间");

                if (WpfConfig.RoomInfo != null && !string.IsNullOrWhiteSpace(WpfConfig.RoomInfo.entity?.entity_id))
                {
                    PluginLog.Info("Core", "检测到用户尚未退出房间, 正在退出...");
                    ...
                    if (exitRoomTask.Wait(TimeSpan.FromMilliseconds(1500)))
                    {
                        ...
                        PluginLog.Debug("Core", $"退出房间返回: {Regex.Escape(sExitRoomResult)}");
                        
                        try 
                        {
                            if (JObject.Parse(sExitRoomResult)["code"].ToObject<int>() == 0)
                                PluginLog.Info("Core", "退出房间成功!");
                            else
                                PluginLog.Error("Core", $"退出房间失败,返回信息:{JObject.Parse(sExitRoomResult)["message"]}!");
                        }
                        catch (Exception ex)
                        {
                            PluginLog.Error("Core", $"解析退房响应失败: {ex.Message}");
                        }
                    }
                    else
                    {
                        PluginLog.Error("Core", "退出房间请求超时，强制结束！");
                    }
                }
            }
            catch (Exception ex)
            {
                PluginLog.Error("Core", $"退出清理时发生异常: {ex.Message}");
            }
            finally
            {
                PluginLog.Info("Core", "清理完成, 正在退出程序...");
                Environment.Exit(0);
            }
        }
        ...
        internal static void InitializeOnLoad()
        {
            ...
            Mcl.Core.Dotnetdetour.Utilities.Diagnostics.LauncherLogging.Configure(false, false, WpfConfig.LauncherRootDirectory);
            ConfigManager.Load();
            ...
            MethodHook.InstallTypes(new[] { typeof(MinecraftPath) });
