...
public unsafe class NativeDetourFor32Bit : IDetour
{
        ...
        MethodBase originalMethod /*原始占位函数*/)
    {
        ...
        if (originalMethod != null) MakePlacholderMethodCallPointsToRawMethod(originalMethod);


        //并且将对原函数的调用指向跳转指令，以此实现将对原始目标函数的调用跳转到用户定义函数执行的目的
        Patch();

        Debug.WriteLine("Patch: Point=" + rawMethod.MethodHandle.GetFunctionPointer().ToInt64() + " Method=" +
                        rawMethod + " Type=" + rawMethod.ReflectedType.FullName);
    }
    ...
    protected virtual void Patch()
    {
        ...
        for (var i = 0; i < newInstrs.Length; i++) *(rawMethodPtr + i) = newInstrs[i];
    }
    ...
    protected virtual void MakePlacholderMethodCallPointsToRawMethod(MethodBase originalMethod)
    {
        ...
        var code = new byte[total_length];
        var ptr = Marshal.AllocHGlobal(total_length);
        //code[0] = 0xcc;//调试用
        for (var i = 0; i < needSize; i++) code[i] = rawMethodPtr[i];
        code[needSize] = 0xE9;
