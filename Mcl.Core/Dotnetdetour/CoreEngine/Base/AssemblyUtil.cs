...
{
    private static readonly object[] m_EmptyObjectArray = new object[] { };
    ...
    public static T CreateInstance<T>(string assembleName, string type)
    {
        ...
        if (instanceType == null)
            return default;
        var instance = Activator.CreateInstance(instanceType, new object[0]);
        result = (T)instance;
        return result;
    }
    ...
    public static T CopyPropertiesTo<T>(this T source, T target)
    {
        ...
        var sourcePropertiesDict = properties.ToDictionary(p => p.Name);
    ...
    public static IEnumerable<Assembly> GetAssembliesFromStrings(string[] assemblies)
    {
        ...
        foreach (var a in assemblies) result.Add(Assembly.Load(a));
    ...
    public static string GetAssembleVer(string filePath)
    {
        ...
        var versionStr = string.Format(" {0}.{1}.{2}.{3}", fvi.ProductMajorPart, fvi.ProductMinorPart,
