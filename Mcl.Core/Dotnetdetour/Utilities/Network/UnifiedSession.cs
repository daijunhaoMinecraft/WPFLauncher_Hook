...
public class UnifiedSession
{
    ...
    private volatile bool _isNeteasePlayer;
    ...
    {
        PeerId = peerId;
        _isNeteasePlayer = isNeteasePlayer;
        LastActive = DateTime.Now;
        _logPrefix = $"[Session:{PeerId}_{ConnId}]";
        Log("=== 构造函数 === 将异步连接本地MC服务器");
    ...
    private void ConnectToLocalMc()
    {
        ...
        try
        {
            ...
            var flushedCount = FlushSendQueue();
    ...
    private void StartReadLoop()
    {
        ...
        try
        {
            while (!_isClosing)
            {
                ...
                var bytesRead = _stream.Read(buf, 0, buf.Length);
                if (bytesRead > 0)
                {
                    LastActive = DateTime.Now;
    ...
    public void SendToSocket(byte[] data)
    {
        ...
        var flushed = FlushSendQueue();
        if (flushed > 0) Log($"[混合发送] 已先发送队列中的 {flushed} 条旧数据");
    ...
    public void Close()
    {
        ...
        // 释放 ID 资源
        WebRtcVar.ConnIdManager.Release(ConnId);
