...
namespace Mcl.Core.Dotnetdetour.HookList
{
	...
	internal class RoomManager : IMethodHook
	{
			...
			if (result.code == 0)
			{
				...
				if (WpfConfig.IsStartWebSocket)
				{
					...
					{ 
						...
						data = new
						{
							entity = WpfConfig.RoomInfo.entity,
							isOwner = true,
		...
		private void v(EntityListResponse<LobbyRoomMemberInfoEntity> result)
		{
			...
			// 获取当前房主ID
			string ownerId = WpfConfig.RoomInfo.entity.owner_id;
		...
		{
			if (WpfConfig.AlwaysSaveWorld && (WpfConfig.RoomInfo.entity.allow_save || WpfConfig.RoomInfo.entity.owner_id == WPFLauncher.Common.azf<arg>.Instance.User.Id.ToString()))
			{
