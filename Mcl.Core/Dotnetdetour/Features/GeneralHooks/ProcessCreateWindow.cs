...
public class BedrockPathWindow_Select : System.Windows.Window
{
    ...
    private void VersionListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        {
            var selectedPath = selectedCard.Tag as string;
...
public class ProcessCreateWindow : IMethodHook
{
    ...
    public static aqq a(string gwa, string gwb, EventHandler gwc, aqk gwd, string gwe = null, bool gwf = false,
        Action<string> gwg = null)
    {
        Console.WriteLine("[StartGame]启动信息创建中...");
        var result = a_original(gwa, gwb, gwc, gwd, gwe, true, gwg);

        // 添加进程退出事件处理，防止CMD窗口关闭
        if (result != null)
        {
            result.EnableRaisingEvents = true;
            result.Exited += (sender, e) => { Console.WriteLine($"\n[进程] 进程 {gwa} 已退出"); };
