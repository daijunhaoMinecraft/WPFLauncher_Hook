...
public class LoggerHook : IMethodHook
{
    ...
    private void SetupCustomNLog()
    {
        ...
        var consoleTarget = new ColoredConsoleTarget("console")
        {
            // 照搬原代码的 Layout
            Layout =
                "${date:format=HH\\:mm\\:ss.fff} | ${level} | ${stacktrace} | ${message} | ${exception:format=tostring}",
        ...
        if (WpfConfig.IsLogOutputFolder) // 假设 WpfConfig 是你的配置类
        {
            ...
            fileTarget.FileName = Path.Combine(logDirectory, "${shortdate}",
                "${cached:${longdate:format=yyyy-MM-dd HH\\:mm\\:ss.fff}:cached=true}.log");

            // 照搬原代码的日志内容 Layout
            fileTarget.Layout =
                "${date:format=HH\\:mm\\:ss.fff} | ${level} | ${stacktrace:separator= > } | ${message} | ${exception:format=tostring,StackTrace}";
