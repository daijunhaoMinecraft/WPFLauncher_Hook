...
{
    public static readonly WintunRouterService Instance = new();


    // 核心路由表：Key = 虚拟IP (如 10.8.0.2), Value = PeerId
    private readonly ConcurrentDictionary<string, string> _routingTable = new();
    ...
    public void Start(string virtualIp)
    {
        ...
        if (WebRtcVar.Mode == ForwardMode.Server)
        {
            ...
            var index = -1;
            var targetUserId = azf<arg>.Instance.User.UserID.ToString();

            for (var i = 0; i < WebRtcVar.PlayerList.Count; i++)
        ...
        {
            if (IPAddress.TryParse(virtualIp, out var ipAddr))
            {
                var ipBytes = ipAddr.GetAddressBytes();

                // 定义头部
                var header = VirualIpProto.MagicHandshake.ToArray();
    ...
    private void CaptureLoop()
    {
        ...
        {
            uint size;
            var packetPtr = WintunReceivePacket(_session, out size);
            if (packetPtr != IntPtr.Zero)
            {
                ...
                {
                    var dstIp = $"{packetData[16]}.{packetData[17]}.{packetData[18]}.{packetData[19]}";
    ...
    {
        var sendPtr = WintunAllocateSendPacket(_session, (uint)data.Length);
    ...
    {
        foreach (var route in _routingTable) SendData(route.Value, data);
    }
