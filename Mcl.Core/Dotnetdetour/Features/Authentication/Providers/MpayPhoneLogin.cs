...
public static class MpayPhoneLogin
{
    ...
    private static void RegisterNewDevice()
    {
        ...
        var url = $"{MPAY_HOST}/mpay/games/{PROJECT_ID}/devices";
        ...
        var formData = new Dictionary<string, string>(GetBaseParams())
        {
            ...
            ["device_name"] = $"PC-{GenerateRandomString()}",
    ...
    public static SmsResult SendSms(string phoneNumber, string deviceId = null)
    {
        ...
        var url = $"{MPAY_HOST}/mpay/api/users/login/mobile/get_sms";
        ...
        try
        {
            ...
            var reason = json?["reason"]?.ToString() ?? $"HTTP {(int)resp.StatusCode}";
    ...
    public static VerifyResult VerifySms(string phoneNumber, string code = "", string upContent = "",
        string deviceId = null)
    {
        var devId = deviceId ?? GetOrRegisterDevice();
        var url = $"{MPAY_HOST}/mpay/api/users/login/mobile/verify_sms";
    ...
    {
        var devId = deviceId ?? GetOrRegisterDevice();
        var encodedPhone = Base64Encode(phoneNumber);
        var url = $"{MPAY_HOST}/mpay/api/users/login/mobile/finish?un={encodedPhone}";
        ...
        try
        {
            ...
            if (resp.StatusCode == HttpStatusCode.OK)
            {
                ...
                var sauthJson = JsonConvert.SerializeObject(sauthDict, Formatting.None);
    ...
    public static string FullLoginFlow(string phoneNumber, string deviceId = null)
    {
        try
        {
            ...
            if (dialogResult != DialogResult.OK || string.IsNullOrEmpty(verifyForm.Ticket))
            {
            ...
            var sauthJson = CompleteLogin(phoneNumber, verifyForm.Ticket, devId);
