...
namespace DotNetTranstor.Hookevent
{
	...
	{
		private static RoomInfoWindow _roomInfoWindow;
		...
		[CompilerGenerated]
		[HookMethod("WPFLauncher.Network.Protocol.LobbyGame.agd", "Left", "Get_Room")]
		// Token: 0x06000433 RID: 1075 RVA: 0x00042D28 File Offset: 0x00040F28
		public static void Left(string roomId)
		{
			if (Path_Bool.AlwaysSaveWorld && (Path_Bool.RoomInfo.entity.allow_save || Path_Bool.RoomInfo.entity.owner_id == azd<arf>.Instance.User.Id.ToString()))
			{
				MessageBoxResult messageBoxResult = uy.q("是否保存存档至云服务端?", "", "确定", "不保存", "");
			...
			{
				if (_roomInfoWindow != null)
				{
					_roomInfoWindow.Close();
					_roomInfoWindow = null;
				}
			});
		...
		public static void SetRoomInfoWindow(RoomInfoWindow window)
		{
			_roomInfoWindow = window;
		}
	}
}
